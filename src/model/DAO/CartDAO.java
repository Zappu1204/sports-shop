...
public class CartDAO {
    ...
    public boolean isCartExistForUser(String userId) {
        String sql = "SELECT 1 FROM Cart WHERE userId = ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, userId);
            ResultSet rs = ps.executeQuery();
            return rs.next(); // Nếu có kết quả, người dùng đã có giỏ hàng
    ...
    public boolean addCart(Cart cart) {
        ...
        if (isCartExistForUser(cart.getUserId())) {
            return false; 
        }
    ...
    private boolean updateProductQuantity(String cartId, String productId, int quantity) {
...
//            String cartId = "KH000003"; // Giả định cartId là "U12345"
//
