...
public class ProductDAO {
    ...
    public boolean addProduct(Product product) throws SQLException {
        // Bước 1: Lấy productId lớn nhất hiện có
        String getMaxIdQuery = "SELECT MAX(CAST(SUBSTRING(productId, 3) AS UNSIGNED)) AS maxId FROM Product";
        try (PreparedStatement maxIdStatement = connection.prepareStatement(getMaxIdQuery);
            ...
            // Bước 2: Tạo productId mới
            int newId = maxId + 1;
            String newProductId = "SP" + String.format("%06d", newId);
            product.setProductId(newProductId);

            // Bước 3: Chèn sản phẩm vào cơ sở dữ liệu
    ...
    public String[][] readProductsTableData() throws SQLException {
        // Bước 1: Đếm số lượng dòng trong bảng Product
        ...
        // Bước 3: Duyệt qua kết quả truy vấn và điền vào mảng hai chiều
        while(result.next()) {
            for(int i = 0; i < columns; i++) {
                products[aux][i] = result.getString(i + 1);  // Cột trong JDBC bắt đầu từ 1
            }
            aux++;
        }
        return products;
